...
const PlaidLinkWidget = () => {
  ...
  if (loading)
    return (
      <div className="plaid-link-widget loading">Fetching Accounts...</div>
    );
  ...
  return (
    <div className="plaid-link-widget">
      <div className="widget-header">
        <h3>
          <Landmark size={18} className="text-blue-400" /> Linked Banking
        </h3>
        <button
          className="btn-link"
          ...
          accounts.map((acc) => (
            <div key={acc.id} className="account-item">
              ...
              <div className="acc-balance">
                <span
                  className={`val ${acc.balance < 0 ? "text-red-400" : "text-green-400"}`}
                >
                  $
      ...
      <div className="widget-footer">
        <span className="security-tag">
          <CheckCircle size={10} /> AES-256 Encrypted
        </span>
