    ...
    <div className="flex flex-col md:flex-row justify-between items-start md:items-center bg-black/40 backdrop-blur-md border-b border-white/5 p-4 rounded-t-lg">
        <div className="flex flex-col">
            ...
            <div className="flex items-baseline gap-3">
                ...
                <span className={`flex items-center text-sm font-mono font-bold ${(dailyPL || 0) >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>
                    {(dailyPL || 0) >= 0 ? <TrendingUp size={14} className="mr-1" /> : <TrendingDown size={14} className="mr-1" />}
                    {(dailyPL || 0) >= 0 ? '+' : ''}${(Math.abs(dailyPL || 0)).toLocaleString(undefined, { minimumFractionDigits: 2 })}
                </span>
            </div>
        </div>
...
const PositionRow = ({ pos }) => {
    ...
    const sparkData = Array.from({ length: 10 }, (_, i) => ({ value: (pos.currentPrice || 0) * (1 + (Math.random() * 0.05 - 0.025)) }));
    const isProfit = (pos.pl || 0) >= 0;
    ...
    return (
        <>
            ...
            >
                ...
                <td className="py-2 font-mono text-slate-300 text-xs text-right pr-4">{pos.qty || 0}</td>
                <td className="py-2 font-mono text-slate-300 text-xs text-right pr-4">${(pos.currentPrice || 0).toFixed(2)}</td>
                ...
                <td className="py-2 text-right pr-3">
                    <div className={`font-mono text-xs font-bold ${isProfit ? 'text-emerald-400' : 'text-rose-400'}`}>
                        {isProfit ? '+' : ''}{(pos.pl || 0).toFixed(2)}
                        <span className="ml-2 opacity-70 text-[10px]">({((pos.pl || 0) / ((pos.qty || 1) * (pos.avgPrice || 1)) * 100).toFixed(2)}%)</span>
                    </div>
                </td>
            </tr>
            {isExpanded && (
                <tr className="bg-black/40">
                    <td colSpan={5} className="p-3">
                        <div className="grid grid-cols-4 gap-4 text-[10px] font-mono">
                            <div className="flex flex-col"><span className="text-slate-500 uppercase">Avg Cost</span><span className="text-slate-300">${(pos.avgPrice || 0).toFixed(2)}</span></div>
                            <div className="flex flex-col"><span className="text-slate-500 uppercase">Market Value</span><span className="text-slate-300">${((pos.qty || 0) * (pos.currentPrice || 0)).toFixed(2)}</span></div>
        ...
        <div className="flex flex-col h-full bg-[#050505] text-white overflow-hidden">
            ...
            <div className="bg-[#111] border-b border-[#222] p-2 flex justify-between items-center text-[10px] font-mono">
                <div className="flex gap-4">
                    <div><span className="text-slate-500 mr-2">EQUITY:</span><span className="text-cyan-400 font-bold">${accountData?.equity?.toLocaleString(undefined, { minimumFractionDigits: 2 }) || '0.00'}</span></div>
                    <div><span className="text-slate-500 mr-2">DAY P/L:</span><span className={`font-bold ${(accountData?.dailyPL || 0) >= 0 ? 'text-emerald-400' : 'text-rose-400'}`}>{(accountData?.dailyPL || 0) >= 0 ? '+' : ''}{(accountData?.dailyPL || 0).toFixed(2)}</span></div>
                </div>
                <div className="flex gap-4">
                    <div><span className="text-slate-500 mr-2">CASH:</span><span className="text-white">${accountData?.liquidity?.toLocaleString(undefined, { minimumFractionDigits: 2 }) || '0.00'}</span></div>
                </div>
            </div>
            ...
            <div className="flex-1 overflow-y-auto custom-scrollbar">
                {activeTab === 'positions' && (
                    <table className="w-full text-left border-collapse">
                        ...
                        <tbody>
                            {(accountData?.positions || []).map((pos, idx) => <PositionRow key={pos.symbol || `pos-${idx}`} pos={pos} />)}
                ...
                {activeTab === 'activity' && (
                    <div className="p-2 space-y-1">
                        {(tradeHistory || []).map((trade, idx) => (
                            <div key={trade.id || `trade-${idx}`} className="flex justify-between items-center bg-[#0a0a0a] p-2 border-l-2 border-[#222] text-[10px] font-mono hover:bg-[#111]">
                                <div className="flex gap-2 items-center">
                                    <span className={(trade.side || 'BUY') === 'BUY' ? 'text-emerald-400' : 'text-rose-400'}>{trade.side || 'BUY'}</span>
                                    <span className="font-bold">{trade.symbol || 'N/A'}</span>
                                    <span className="text-slate-500">{trade.qty || 0} @ ${(trade.price || 0)}</span>
                                </div>
                                <span className="text-slate-600">{trade.time || '--:--'}</span>
                            </div>
                        ))}
                        {(!tradeHistory || tradeHistory.length === 0) && (
                            <div className="py-8 text-center text-slate-600 font-mono text-[10px] uppercase">No Recent Activity</div>
                        )}
                    </div>
                )}
            </div>
