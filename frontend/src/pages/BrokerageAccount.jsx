...
const BrokerageAccount = () => {
    ...
    const heroStats = [
        { label: 'Total Liquidity', value: summary?.liquidity || 0, prefix: '$', status: 'positive', sparklineData: equityData.slice(-10).map(d => d.Value) },
        { label: 'Day P&L', value: Math.abs(summary?.dailyPL || 0), prefix: (summary?.dailyPL || 0) >= 0 ? '+$' : '-$', change: (summary?.dailyPL || 0) >= 0 ? 2.4 : -1.8, changeLabel: 'vs yesterday', status: (summary?.dailyPL || 0) >= 0 ? 'positive' : 'negative' },
                ...
                <div key="stats" className="flex gap-4">
                    {heroStats.map((stat, idx) => (
                        <div key={idx} style={{ flex: 1 }}>
                            <StatCard 
                                {...stat}
                                formatValue={v => stat.prefix?.includes('$') ? (v?.toLocaleString() || '0.00') : (v || '0')}
                            />
                        </div>
                    ))}
                </div>
                ...
                <div key="positions">
                    <GlassCard variant="default" hoverable={false} className="h-full overflow-hidden">
                        <div className="glass-card-drag-handle cursor-move flex justify-between items-center mb-4">
                            ...
                            <Badge count={summary?.positions?.length || 0} variant="info" />
                        </div>
