...
_dept_connections = {} # Dict[int, Set[sid]]

# Constants
HEARTBEAT_INTERVAL = 30
MAX_CONNECTIONS_PER_DEPT = 50

def _verify_jwt(token: str) -> Optional[Dict]:
    """Verify JWT token using auth_utils."""
    from web.auth_utils import decode_token
    return decode_token(token)


_dept_sio = None
_dept_socket_app = None

def get_dept_sio():
    global _dept_sio
    if _dept_sio is None:
        import socketio
        _dept_sio = socketio.AsyncServer(
            async_mode='asgi',
            cors_allowed_origins=["http://localhost:5173", "http://127.0.0.1:5173", "*"],
            logger=False,
            engineio_logger=False
        )
        _register_dept_handlers(_dept_sio)
    return _dept_sio

def get_dept_socket_app():
    global _dept_socket_app
    if _dept_socket_app is None:
        import socketio
        _dept_socket_app = socketio.ASGIApp(get_dept_sio(), socketio_path='')
    return _dept_socket_app

def _register_dept_handlers(sio):
    @sio.event
    async def connect(sid: str, environ: Dict, auth: Optional[Dict] = None) -> bool:
        ...
        if token:
            ...
            if payload:
                ...
                await sio.save_session(sid, {'user_id': 'anonymous', 'authenticated': False})
        else:
            await sio.save_session(sid, {'user_id': 'anonymous', 'authenticated': False})
    ...
    async def subscribe_department(sid: str, data: Dict[str, Any]) -> None:
        ...
        if not dept_id or not isinstance(dept_id, int):
            await sio.emit('error', {'message': 'Invalid dept_id'}, to=sid)
            return
    ...
    async def unsubscribe_department(sid: str, data: Dict[str, Any]) -> None:
        ...
        if dept_id and dept_id in _dept_connections:
            ...
            await sio.emit('unsubscribed', {'dept_id': dept_id}, to=sid)
