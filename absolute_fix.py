...
for line in orig_lines:
    ...
    if in_create_app:
        ...
        # Strip all web.api/web.routes imports within create_app
        if 'from web.api' in line or 'from web.routes' in line or (stripped.startswith('from') and 'import' in line and ('_bp' in stripped or 'api' in stripped)):
            new_lines.append("    # " + stripped + " # ABSOLUTE STRIP\n")
            continue
            
    # Handle top-level imports - replace with consolidated block
    if not in_create_app and ('from web.api' in line or 'from web.routes' in line or '# --- MASTER' in line or '# --- FINAL' in line or '# --- AUTOMATICALLY' in line):
        if not imports_injected:
            new_lines.append("\n# --- ABSOLUTE CONSOLIDATED IMPORTS ---\n")
            for imp in new_imports:
                new_lines.append(f"{imp}\n")
            new_lines.append("# --- FINISHED CONSOLIDATION ---\n\n")
...
# Absolute fallback for injection
if not imports_injected:
    new_lines.insert(25, "\n# --- ABSOLUTE CONSOLIDATED IMPORTS (FALLBACK) ---\n")
    for imp in new_imports:
        new_lines.insert(26, f"{imp}\n")
    new_lines.insert(26 + len(new_imports), "# --- FINISHED CONSOLIDATION ---\n\n")
